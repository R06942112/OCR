...
def load_img_label(path,img_size,decoder_length):
    result = []   
    result_l = []
    result_m = []
    result_n = []
    f_size = int(img_size/8)
    for p in path: 
        image = cv2.imread(p)
        resized = cv2.resize(image, (img_size,img_size), interpolation = cv2.INTER_AREA)
        #resized = cv2.equalizeHist(resized)    
        ...
        p = p.replace("SynthTextCrop", "SynthTextAtt")
        npy = np.load(p[:-3]+'npy')
        num = npy.shape[0]
        mask = np.ones((num,f_size,f_size))
        
        mask = np.append(mask, np.zeros((decoder_length - num,f_size,f_size)), axis=0)
        npy = np.append(npy, np.zeros((decoder_length - num,f_size,f_size)), axis=0)
        result_l.append(npy)
        result_m.append(mask)
        
        p = p.replace("SynthTextAtt", "SynthTextMask")
        image = cv2.imread(p, cv2.IMREAD_GRAYSCALE)
        resized = cv2.resize(image, (img_size,img_size), interpolation = cv2.INTER_AREA)
        normalized = (resized / 255).astype(np.float32)
        result_n.append(normalized)
        
    return np.array(result),np.array(result_l),np.array(result_m),np.array(result_n)
